...
class db_class {
   ...
   function select_db($db='') {
    ...
    // performs a mysql_num_rows against the $sql
    $this->last_query = $sql;
...
function ind_dump($sql,$title) {
      ...
      while ($row = mysql_fetch_assoc($r)) {
         if ($i == 0) {
            echo "<tr><td colspan=\"".sizeof($row)."\" class=\"center title\">$title</td></tr>\n";
            echo "<tr>\n";
            foreach ($row as $col => $value) {
	      switch ($col){
			case "fname":
			...
			case "datebirth":
			  ...
			  break;
			case "datelast":
			  $cols = "Date of Last Visit";
			  break;
			case "course":
			  $cols = "Course";
			  break;
			case "misc":
			  $cols = "<acronym title=\"Authorised Contact(Visitor)  |  Contractor # and Business Name(Contractor)  |  QantasLink Employee #(QantasLink Employee)\">Extra Information (hover here for details)</acronym>";
			  break;
	      }
	      if($col!="misc" && $col!="Misc"){
	      	if(ISSET($_GET['sort']) && ($_GET['sort'] == $col)) {
			// reverse direction
				if(!$_GET['sortdir'] || $_GET['sortdir']=='DESC') { $sortdir="ASC"; } else { $sortdir="DESC"; }
	      	} else {$sortdir="ASC";}
               	echo "<td class=\"semititle center\"><strong><a href=\"?report=".$_GET['report']."&sort=$col&sortdir=$sortdir\">$cols</a></strong></td>\n";
	      } else {
	      	echo "<td class=\"semititle center\"><strong>$cols</strong></td>\n";
	      }
           }
            echo "</tr>\n";
         }
	...
	$u = 0;
         foreach ($row as $value) {
	    if($u == 2) {
	      // we have the datebirth situation
	      $value = substr($value,0,2)."-".substr($value,2,2)."-".substr($value,4,4);
	    }
	    if($u == 3) {
	      // convert datelast to human-dates
	      if($value!=0){
	      $value = date('d-m-Y',$value);} else { $value = "<em>incomplete</em>";}
	    }
	    if(!$value){$value="&nbsp;";}
            echo "<td bgcolor=\"$bg\">".ucwords(strtolower($value))."</td>\n";
	    $u++;
         }
         echo "</tr>\n";
      }
echo "<tr onMouseOver=\"this.style.backgroundColor='#eeeeee'\"; onMouseOut=\"this.style.backgroundColor='#dddddd'\";>
       <td colspan=\"6\" class=\"center\"><input type=\"submit\" name=\"leaveadmin\" class=\"inputbutt\" value=\"Back to Admin Menu\" onClick=\"javascript:window.location='index.php';\" /> </td></tr>";
      echo "</table>\n";
   }
